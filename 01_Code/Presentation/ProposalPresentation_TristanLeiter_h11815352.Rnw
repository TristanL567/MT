...
\begin{frame}{The "Agony and Ecstasy" of Indexing}
  \begin{itemize}
    ...
    Indices capture the "Ecstasy" of \\
    extreme winners but systematically force investors to hold the "Agony" of \\
    imploding stocks.
    \begin{itemize}
        \item \citet{Cembalest2014} finds that the Russell 3000 Index is driven by a small tail of extreme winners, 
        while approx. 40\% of constituents are negative return contributors, with many suffering "catastrophic declines" without recovery.
    \end{itemize}

    \item \citet{Tewari2024} formalize these events as \\
...
\begin{frame}{Methodology II: Modelling}
  \begin{itemize}
    ...
    \item Using \textbf{Autoencoders} on the feature engineered data to test their utility for either feature denoising or signal generation.
...
\begin{frame}{Methodology III: Index Construction}
  ...
  All firms exceeding the probability threshold, $\theta$, will be removed.
  In this context, $\theta$ is selected based on the desired FPR rate (for example $3\%$ or $5\%$).
  Additionally, the "Crash-Filtered" Index will be rebalanced annually at the end of each calendar year.
...
\begin{frame}{Expected Contribution}
  ...
  \begin{itemize}
    \setlength\itemsep{0.6em}
        ...
        of the informational content in the features and by using the model-predictions for index construction.
...
\section{Appendix}
\begin{frame}{Appendix: Frequency of Firms by CSI Count}
...
\begin{frame}{Appendix: Annual Frequency and Percentage of CSI Events}
...
\cellcolor{gray!10}{2010} & \cellcolor{gray!10}{70} & \cellcolor{gray!10}{3.13} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{}\\
\bottomrule
\end{tabular}
% --------------------------
\captionsetup{font=tiny}
