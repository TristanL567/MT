...
Directory <- file.path(Path, "01_Code/Presentation")
Directory_LaTeX <- file.path(Directory, "ProposalPresentation_TristanLeiter_h11815352.Rnw")
setwd(Directory)
...
\definecolor{grey}{RGB}{197,204,211} 

% 2. Force Titles to be White on Red
\setbeamercolor{title}{bg=beaverRed, fg=white}
\setbeamercolor{frametitle}{bg=beaverRed, fg=white}
\setbeamercolor{subtitle}{fg=white} % Subtitle on title page

% 3. Force Footer to be White on Red
\setbeamercolor{date in head/foot}{bg=beaverRed, fg=white} 
\setbeamercolor{footline}{bg=beaverRed, fg=white}

%%%% Adjusts the text boxes (alterboxes) %%%%
\setbeamercolor{block title alerted}{bg=beaverRed, fg=white}
...
\begin{frame}{Methodology II: Modelling}
  ...
  Since the dataset is imbalanced, AP provides for a more robust signal for hyperparameter
...
\begin{frame}{Methodology III: Index Construction}
  ...
  The best-performing model will within the test-set be used to predict a CSI for the consecutive year.
  The "Crash-Filtered" Index will be rebalanced anually at the end of each calendar year.
  The probability threshold, $\theta$, is selected based on the desired FPR rate (for example $3\%$ or $5\%$).
...
\begin{frame}{CRSP}
      ...
      \begin{itemize}
        ...
        \item \textbf{Size:} $3,255$ unique firms, 46,294 firm-year observations.
        ...
        \item \textbf{CSI-Events:} $2,507$ CSIs in Total. 
      \end{itemize}
...
\cellcolor{gray!10}{Value Destruction (Intermitent Recovery)} & \cellcolor{gray!10}{48.5\%} & \cellcolor{gray!10}{16.8\%}\\
\addlinespace
Value Destruction (No Recovery) & 1.4\% & 1.1\%\\
\cellcolor{gray!10}{Unclassified} & \cellcolor{gray!10}{11.1\%} & \cellcolor{gray!10}{7.6\%}\\
\bottomrule
