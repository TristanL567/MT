...
\textit{Does the integration Autoencoders significantly improve the performance (Average Precision) of Ensemble models 
    compared to  those trained solely on raw financial data?} \\
